...
export default function MainPlayerPage() {
  ...
  const { setOrientationPermission } = usePermission()
  ...
  // Handle events from LandingPage
  const handleLandingPageEvent = async (event: LandingPageEvent) => {
    console.log("LandingPage event:", event)
    switch (event.type) {
      case "DESKTOP_BEGIN_TOUR":
        setShowLandingPage(false)
        // Start audio playback if not already playing and audio is ready
        if (audioRef.current && audioRef.current.readyState >= 2 && audioRef.current.paused) {
          audioRef.current.play().catch(err => console.error("Error playing audio post-landing:", err))
        }
        break
      case "MOBILE_BEGIN_TOUR":
        // The permission itself (granted/denied) would have been stored by landing-page/utils
        // We just proceed to hide the landing page.
        // xr-video-player will pick up the permission status from localStorage.
        setShowLandingPage(false)
        setOrientationPermission(event.permissionGranted ? "granted" : "denied") // Update context
        ...
        break
      case "PERMISSION_RESULT": // This event might be redundant if MOBILE_BEGIN_TOUR includes grant status
        // but we can use it to ensure context is updated.
        setOrientationPermission(event.granted ? "granted" : "denied")
        break
    }
  }
  ...
  if (showLandingPage) {
    return (
      <LandingPage
        onEvent={handleLandingPageEvent}
        title="Welcome to the Immersive Experience"
        description="Prepare to explore a new dimension of sound and visuals. Please grant device orientation access for the full XR experience on mobile."
        tourButtonText="Begin Experience"
      />
    );
  }
  ...
  return (
    <div className="min-h-screen h-full bg-background overflow-hidden">
      ...
      {/* Preload XR video in the background (invisible) when isXRTrack is true */}
      {isXRTrack && (
